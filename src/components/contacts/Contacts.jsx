...
const sendEmail = (e) => {
  ...
  emailjs.sendForm('service_6a6jtfe', 'template_g2cpat3', formRef.current, 'x73f3UMHgIejzpbGh')
    .then((result) => {
       setSuceess(true)
    }, (error) => {
        setError(true)
    ...
    return (
      <motion.div
      ref={ref} 
      key='contacts' className='contact' variants={variants} initial="initial" animate="animate">
          <div className='textContainer' variants={variants}>
            
              <h1><span className='letter'> L</span>et's <br/> Collab</h1>
          </div>
  
          <div className='formContainer'>
          <motion.div
        //   ref={svgRef}
             className="phoneSvg" initial={{opacity:1}} whileInView={{opacity:0}} transition={{ delay:3 ,duration:1}}
